...
type Options = { input :: FilePath, output :: FilePath }
...
useOptions { input, output } = launchAff_ do
...
extractClassNames css = case f <$> res of
  Left e -> do
    let Position { column, line } = parseErrorPosition e
    log $ Array.fold
      [ "Parse error at "
      , show line
      , ":"
  ...
  res = runParser css Parser.classNames
    <#> Array.sort <<< Array.filter isValidClassname

  isValidClassname :: String -> Boolean
  isValidClassname cn =
    not (String.startsWith "-" cn)
      && not (String.startsWith " " cn)

  f :: Array String -> Array (Tuple String String)
  f ss = Array.nubBy (compare `on` fst) (toTuple <$> ss)
